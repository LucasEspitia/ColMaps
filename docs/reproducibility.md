...
`mambaorg/micromamba` was selected as the base image because the data pipeline already defines its Python dependencies through a Conda `environment.yml` file. This allows the same dependency specification used during local development to be reused inside Docker, reducing duplicated configuration and improving consistency between local and containerized environments.

Micromamba also provides a lightweight alternative to full Conda distributions while remaining compatible with the pipeline's geospatial dependencies and `conda-forge` packages.
...
### 11.2 Database Import Containerization

The validated PostGIS import workflow was containerized as a dedicated one-shot service responsible for loading the processed OSM dataset into the database.

#### 11.2.1 Dockerfile

A dedicated Dockerfile was added in:

```text
database/Dockerfile
```

The container reuses the existing database automation environment defined in:

```text
database/environment.yml
```

and also includes `osm2pgsql`, which is required to import the prepared OSM dataset into PostGIS.
...
The existing import validation and skip logic is therefore preserved inside the containerized workflow.

#### 11.2.2 Shared Processed Dataset Volume

The import container requires access to the final dataset generated by the data pipeline:

```text
data-pipeline/processed/colombia-colmaps.osm.pbf
```

Rather than copying this file into the database image, the existing persistent processed-data volume is shared between the two services:
...
This allows the data pipeline to generate the dataset once and the import service to consume the same artifact without duplicating it.

#### 11.2.3 Docker Compose Integration

The root `docker-compose.yml` was extended with the `data-import` service:
...
data-import:
  ...
  container_name: colmaps-data-import

  environment:
    DB_HOST: db
    DB_PORT: ${DB_PORT:-5432}
    DB_NAME: ${DB_NAME:-colmaps}
    DB_USER: ${DB_USER:-colmaps}
    DB_PASSWORD: ${DB_PASSWORD:-colmaps_pwd}
...
Inside the Docker network, the PostGIS service is accessed through:
...
The dependency configuration ensures that the import service starts only after:
